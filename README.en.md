...
- [Chart `commons`](#chart-commons)
  - [Overview](#overview)
  - [`secrets` (sensitive values)](#secrets-sensitive-values)
  - [`global`](#global)
...
Optional root section gathering **all** the sensitive values of the release, referenceable from anywhere in the values through the `__secrets__<key>` placeholder:
...
It is designed to be **isolated in an encrypted file** ([SOPS](https://github.com/getsops/sops), [helm-secrets](https://github.com/jkroepke/helm-secrets), …) and merged at deploy time, while the rest of the values stays readable and diffable in plain text in git:

```bash
helm secrets upgrade --install myapp dou-charts/commons \
  -f values.yaml \
  -f secrets.yaml   # holds only the `secrets` section, encrypted
```

It is a **root-level map**, unlike `components` which is a list: Helm merges maps deeply but **replaces** lists. Without this section, isolating the password of a `components[].postgres.cluster` in a separate encrypted file would require duplicating the whole list item into it.

Where `__secrets__<key>` is resolved:

| Location | Example |
|---|---|
| `env[].value` and `env[].valueFrom.secretKeyRef.name` (deployment, cronjob, job) | `value: __secrets__appApiKey` |
| `components[].secrets[].data.<key>` | `data: { apiKey: __secrets__appApiKey }` |
...
Resolution is also **transitive**: a placeholder pointing at a value that is itself sensitive works. For instance `__components__postgres__password` returns `component.postgres.cluster.password`, and if that one is set to `__secrets__pgPassword`, the key is resolved along the way.

Things to know:

- A key that is referenced but missing from `secrets` makes the rendering **fail** with an explicit message, rather than silently producing an empty value.
- Key names cannot contain `__` (it is the placeholder separator); the values schema rejects them.
- Values are deliberately **not resolved in the ConfigMaps** generated by `volumes[].configMap`: a ConfigMap is not meant to carry a secret. Use `components[].secrets` for that.
- This section is not a replacement for a cluster-side secret manager: the decrypted value goes through Helm and stays readable via `helm get values <release>` (it is stored in the release Secret). For the secret to exist only in the cluster, you need an `ExternalSecret` / `SealedSecret` and to reference the resulting Secret through `env[].valueFrom.secretKeyRef`.
...
| `__addons__postgres__password_secret` | Name of the `<release>-postgres-secret` Secret |
...
| `__addons__redis__password` | `addons.redis.password` in plain text |
| `__addons__redis__password_secret` | Name of the Secret holding the Redis password (`password` key) |
...
| `__secrets__<key>` | `secrets.<key>` — see [`secrets`](#secrets-sensitive-values). Resolved last, including on the result of another placeholder. |
...
| `cluster.password` | — | Accepts `__secrets__<key>` to keep the password out of the values in plain text (see [`secrets`](#secrets-sensitive-values)). |
