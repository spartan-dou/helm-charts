...
{{- if .Values.addons.pgAdmin.enabled }}
  {{- $defaults := dict
    ...
    "configMap" (list (dict
      "name" "pg-config"
      "data" (dict
        "server.json" (indent 4 (include "pgadmin.servers" (dict "Values" $.Values "Chart" $.Chart "Release" $.Release)))
        "pgpass" (indent 4 (include "pgadmin.pgpass" (dict "Values" $.Values "Chart" $.Chart "Release" $.Release)))
      )
    ))
    "service" (dict
      "enabled" true
